...
class ReviewControllerImplTest {
    ...
    @Test
    void getByRecipeId() throws Exception {
        MvcResult result = mockMvc.perform(get("/api/v1/reviews/recipe/" + review1.getRecipeId()))
    ...
    void getByUserId() throws Exception{
        MvcResult result = mockMvc.perform(get("/api/v1/reviews/user/" + review1.getRecipeId()))
                .andExpect(status().isOk())
                .andReturn();
        assertTrue(result.getResponse().getContentAsString().contains(review1.getTitle()));
        assertFalse(result.getResponse().getContentAsString().contains(review5.getTitle()));
